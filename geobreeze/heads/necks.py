...
# copied with minimal adjustments from https://github.com/open-mmlab/mmsegmentation/blob/main/mmseg/models/necks/featurepyramid.py
        ...
        for k in self.rescales:
            if k == 4:
                self.upsample_4x = nn.Sequential(
                    ...
                    nn.BatchNorm1d(embed_dim), # build_norm_layer(norm_cfg, embed_dim)[1], (norm_cfg=dict(type='SyncBN', requires_grad=True))
                    nn.GELU(),
                    nn.ConvTranspose2d(
                        embed_dim, embed_dim, kernel_size=2, stride=2),
                )
